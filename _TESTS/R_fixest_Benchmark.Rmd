---
author: "adapted from Laurent Berge by Miguel Portela"
...
This is an adaptation of Laurent Berge's benchmark code. Full information available [here](https://github.com/lrberge/fixest).
...
# setwd("/Users/miguelportela/Documents/GitHub/HDFE/")
library(data.table)
library(MASS)
library(lfe)
library(glmmML)
...
```{r}
#load("_BENCHMARK/DATA/base_all_simulations.Rdata")

nlswork_regression <- read_dta("~/Documents/GitHub/HDFE/nlswork_regression.dta")
```

```{r}
HDFE4 <- feols(ln_wage ~ ttl_exp + union + not_smsa + nev_mar| idcode + year, data = nlswork_regression)
...
```{r A.4.1.1 Combining several fixed-effects (fe1^fe2^fe3...)}
...
```{r, eval=FALSE}
library(data.table)
base = fread("_BENCHMARK/DATA/base_10M.csv")
...
data.table::fwrite(results_all, file = "_BENCHMARK/DATA/results_bench_R.txt")
...
load("_BENCHMARK/DATA/base_all_diff.Rdata")
...
data.table::fwrite(results_all_diff, file = "_BENCHMARK/DATA/results_diff_bench_R.txt")
