...
```{r Wages PLM}
wages_plm <- plm(ln_wage ~ ttl_exp + union + not_smsa + nev_mar, model="within",nls, index=c("idcode","year"))
summary(wages_plm)
```

```{r Wages FEOLS - VCOV cluster}
# Stata: reghdfe ln_wage ttl_exp union not_smsa nev_mar, absorb(idcode year) vce(cluster idcode)
...
wages_feols <- feols(ln_wage ~ ttl_exp + union + not_smsa + nev_mar | idcode + year,nls,fixef.tol = 1e-6,fixef.iter = 200,fixef.rm = "singleton", vcov = "iid")
summary(wages_feols)
# print(wages_feols)
```

```{r}
summary(wages_feols, vcov = "twoway")
```
...
# etable(gravity_ols,
#          vcov = "twoway", headers = c("Gaussian"))
